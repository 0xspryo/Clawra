/**
 * CLAWRA Database Schema & Validation Types
 * 
 * Defines the complete data model for the CLAWRA autonomous agent
 * evolution system and Solana wallet distributor.
 * 
 * Schema design principles:
 * - Agents use soft deletes (status field) to preserve lineage history
 * - Self-referencing parentId enables lineage tree reconstruction
 * - All continuous parameters stored as real (float4) for mutation precision
 * - Strategy stored as text[] array for multi-strategy agent configurations
 * - Trades support both open and closed states for future streaming
 * - Mutations link agents to cycles for temporal lineage analysis
 * 
 * @module schema
 * @author CLAWRA
 */
 ...
 * 
 * Each agent represents an autonomous trading entity with:
 * - A unique Solana wallet (Ed25519 keypair)
 * - 1-3 trading strategies from the STRATEGIES pool
 * - Continuous parameters that undergo genetic mutation during evolution
 * - Performance metrics updated after each evaluation cycle
 * - Lineage tracking via parentId and generation number
 */
 ...
 * 
 * Each trade records a simulated market position taken by an agent.
 * Supports both long and short directions across Solana ecosystem tokens.
 * P&L is calculated at trade close based on entry/exit price differential.
 */
 ...
 * 
 * Records the outcome of each evolution cycle including population
 * statistics, selection events, and the best/worst performing agents.
 * Used to visualize evolutionary progress over time.
 */
...
/**
 * Parameter Mutation Records
 * 
 * Tracks every parameter change during the cloning process.
 * Links agent → cycle to enable temporal analysis of how
 * specific parameters evolved across generations.
 * Essential for the lineage tree and mutation history views.
 */
 ...
 * System Activity Log
 * 
 * Records all significant system events including agent births,
 * deaths, cloning, evolution cycles, and system messages.
 * Powers the real-time activity feed on the dashboard.
 */
...
/**
 * Available trading strategies for agents.
 * Each strategy represents a different market approach:
 * - momentum: Follow price trends
 * - mean_reversion: Bet on price returning to average
 * - sentiment: Trade based on social/market sentiment signals
 * - sniper: Quick entry/exit on sharp moves
 * - arbitrage: Exploit price differences across markets
 * - liquidity_tracking: Follow large liquidity movements
 * - meme_velocity: Trade meme tokens based on viral velocity
 */
...
/**
 * Solana ecosystem tokens available for trading simulation.
 * Includes major DeFi tokens and popular meme coins.
 */
