 ...
 * 
 * Registers all REST endpoints for both the Solana wallet distributor
 * and the CLAWRA autonomous agent evolution system.
 * 
 * Security: All CLAWRA agent responses are sanitized via sanitizeAgent()
 * to strip walletPrivateKey before sending to clients. Private keys never
 * leave the server boundary.
 * 
 * Wallet distributor endpoints handle real Solana blockchain operations
 * (keypair generation, balance queries, SOL transfers) via @solana/web3.js.
 * 
 * @module routes
 * @author CLAWRA
 */
...
/**
 * Strip wallet private key from agent data before sending to clients.
 * This is the security boundary — no private key data should ever
 * cross into API responses.
 */
